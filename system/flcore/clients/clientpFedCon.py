...
class clientpFedCon(Client):
    ...
    def save_last_local_model(self):
        self.last_local_model = copy.deepcopy(self.model_per)
        ...
        for step in range(max_local_epochs):
            for x, y in trainloader:
                ...
                if self.last_local_model !=0:
                    ...
                    labels = torch.ones(x.size(0)).cuda().long() #拉向本地模型，因此标签是 1

                    moon_loss = self.lam * self.loss(logits, labels)
                    loss += moon_loss
                
                #contrastive loss based on samples 
                _, out_1, _  = self.model_per(x1)
                _, out_2, _ = self.model_per(x2)
                # [2*B, D]
                out = torch.cat([out_1, out_2], dim=0)
                # [2*B, 2*B]
                sim_matrix = torch.exp(torch.mm(out, out.t().contiguous()) / self.tem)
